...
public:
    ...
    void addEdge(int id_1, int id_2){
        if (!nodes[id_1].is_existed){
            ...
            memcpy(new_neighbors, nodes[id_1].neighbors, sizeof(int) * (nodes[id_1].degree));
        ...
        if (!nodes[id_2].is_existed){
            ...
            memcpy(new_neighbors, nodes[id_2].neighbors, sizeof(int) * (nodes[id_2].degree));
        ...
        //printf("%d %d %d %d\n", id_1,nodes[id_1].degree, id_2, nodes[id_2].degree);
        return;
    }
    ...
    // return value is the number of common neighbors, neighbors are stored in common_neighbors
    ...
    void calculateAllNodeLCC(){
        ...
        for(id = 0;id <= max_node;id ++){
            if (nodes[id].is_existed){
                ...
                if(nodes[id].lcc >= max_lcc)
                    max_lcc = nodes[id].lcc;
            }
        }
    ...
    void calculateUpperBound(int k){
        ...
        for (id = 0; id <= max_node; id++){
            if (nodes[id].is_existed){
                ...
                for (i = 0; i <= k;i++)
                    nodes[id].upper_bound[i] = 0;
                
                for (k1 = 0; k1 <= k; k1++){
                    for(i = k1; i <= k; i++){
                        current = (count + (i - k1) + k1 * nodes[id].degree)/ (nodes[id].degree + k) / (nodes[id].degree+ k -1) *2;
    ...
    void updateForAnNewEdge(int u, int v){
        ...
        for (i = 0; i < n_common; i++){
            ...
            nodes[common_neighbors[i]].lcc += 2.0/nodes[common_neighbors[i]].degree/(nodes[common_neighbors[i]].degree -1);
        }
        #pragma omp barrier
    ...
    void updateForAnNewEdgeA(int u, int v, int k){
        ...
        for (i = 0; i < n_common; i++){
            ...
            if (nodes[common_neighbors[i]].upper_bound[k] > max_lcc){
                ...
                nodes[common_neighbors[i]].lcc = nodes[common_neighbors[i]].upper_bound[k];
            }
        }
    ...
    double BUM (int k, char *output_file){
        ...
        for (i = 1; i <= k; i++){
            ...
            for(j = 0; j < max_node; j++){
                ...
                if(nodes[j].is_existed && j != v_m && !nodes[v_m].hasNeighbor(j) && nodes[j].lcc >current_max){
            ...
            if (i % 100 == 0){
                ...
                t3 = clock();
                calculateAllNodeLCC();
                printf("BUM\t%d\t%.2lf\t%.2lf\n", i, returnMaxLCC(), double(t3-t2)/CLOCKS_PER_SEC);
            }
	...
	double OISA(int k, char *output_file){
		int id, i, max_d, k_G;
		double l, a, b, c, temp;
		double min_max_lcc, min_l, return_;
		char file_name[128];
		...
		for(i = 1; i <= max_d; i++){
			...
			for(id = 0; id <= max_node; id++){
				...
				if (nodes[id].is_existed && nodes[id].lcc >= l){
					...
					c = (nodes[id].degree - 1) * nodes[id].degree * (l * nodes[id].degree - nodes[id].lcc);
			...
			if (k_G < 2 * k && k_G > 0){
				sprintf(file_name, "%.2lf_%s\0", l, output_file);
...
int main (int argc, char ** argv){
    ...
    if (argc != 5){
        printf("USAGE: <executable-file> <algorithm> <input-network> <lcc-file> <k>\n");
        return 0;
    }
    ...
    sprintf(output_file_name, "%s_%s_%d.txt\0", argv[1], argv[2], k);
    ...
    //graph.test();
    printf("%s\t%d\t%.2lf\t%.2lf\n", argv[1], k, obj, double(t3-t2)/CLOCKS_PER_SEC);
    return 0;
}